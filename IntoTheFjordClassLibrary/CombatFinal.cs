...
namespace IntoTheFjordClassLibrary
{
    public class CombatFinal
    {
        ...
        public static void DoBattle(CompletedPlayer player, Enemy enemyFinal)
        {
            Thread.Sleep(500);
            DoAttackPlayer(player, enemyFinal);
            Thread.Sleep(1000);

            if (enemyFinal.Life > 0)
            {
                Console.WriteLine($"The {enemyFinal.Name} closes in on you for an attack.");
                DoAttackEnemy(enemyFinal, player);
            }
        }
